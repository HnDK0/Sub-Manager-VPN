...
вредоносные узлы, пропинговывает каждый кандидат 3× через реальный подпроцесс
**xray-core** (16–32 параллельных воркера), оставляет 3–5 лучших по стране по
...
Ядра подгружаются автоматически из официальных релизов и проверяются по SHA.
...
- [Планировщик 24/7 и авто-загрузка ядер](#планировщик-247-и-авто-загрузка-ядер)
...
- **Linux (Ubuntu 22.04 или новее)**. Код кроссплатформенный, но egress-изоляция
  и пути к ядрам заточены под Linux.
- **Go 1.25+** (для сборки из исходников). Бинарник внешних зависимостей не требует.
- **Доступ в интернет** (HTTPS): менеджер скачивает xray-core, базу GeoIP и сами
...
2. Скачивает **xray-core** (последняя стабильная версия, проверка SHA256) в `cores/`.
...
| `cores/` | Скачанные бинарники xray-core (и опц. sing-box), проверенные по SHA |
...
맞는 формату. По умолчанию слушает на `127.0.0.1:18080` (переопределяется
...
| `-cores` | `…/cores` | каталог бинарников ядер (xray/sing-box) |
...
## Планировщик 24/7 и авто-загрузка ядер
...
- **Ядра**: `core.Manager` при старте скачивает последнюю **стабильную** версию
  xray-core (опц. sing-box) из GitHub Releases и сверяет SHA256. Бинарник
  кешируется в `cores/`.
...
- **Самовосстановление**: мёртвые/деградирующие узлы вытесняются живыми кандидатами
  ...
  пробы. На первом цикле часть узлов может прийти «мёртвыми», пока xray не
  поднялся, — последующие циклы исправляют это сами (см. ниже).
...
- **Все узлы мертвы на самом первом цикле** — скорее всего, xray ещё не успел
  поднять SOCKS5-порт, когда движок начал пинговать. Это **исправлено**: движок
  теперь ждёт, пока xray реально привяжет порт, прежде чем слать пробы. Если видишь
  это на очень медленной машине, подними `proxyReadyTimeout` в `internal/test/engine.go`
...
Интеграционный тест `internal/integration` (`TestEngineSmokeNoXray`) прогоняет весь
путь воркеров через in-process fake-SOCKS5 без xray/сети. Реальный путь
(`TestIntegrationRealXray`) скачивает xray и чисто пропускается, если сеть/загрузка
недоступны.

Пайплайн (6 стадий + подсистемы):

```
whitelist (config) → fetch → parse → filter → test (ping через xray) → select → generate (3 формата)
                          │                                                        │
                          └────────── state (SQLite) ◄── scheduler (24/7) ◄───────┘
                                          │
                                    core (бинарь) + geo (mmdb)
```
